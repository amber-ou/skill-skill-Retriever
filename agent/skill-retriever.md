...
- 維護觸發：使用者直接呼叫，或 Agent Office（`amber-ou/agent-office` repo 的 Claude Code
  工作階段 SessionStart）開啟時；不建立固定排程。
...
使用者直接呼叫，或 `amber-ou/agent-office` 的 Claude Code 工作階段開始（SessionStart）時，
觸發既有目錄的查核與更新。首次呼叫可執行 UI／UX 試收錄；後續新增蒐集依使用者指定任務進行，
不在每次開啟時無限制擴張清單。一般跨 Agent 查詢不強制完整掃描，可先回傳現有資料及新鮮度，
需要更新時才呼叫 `refresh_skills`。相同觸發重複送達或已有維護進行中時合併工作。

檢查來源、版本、授權、指標與關係；內容變更時重新分析。來源移除／封存／失效時標記狀態並
保留歷史。分開記錄「最後嘗試查核」與「最後成功查核」；失敗保留舊有效數據、標示過期與原因，
不偽裝成成功或寫成零值。

使用者直接提出的任務正常回覆結果；Agent Office 開啟觸發的背景維護，只有重要變更、失效、
重試後仍失敗或需要決策時才通知，沒有重要變化時保持安靜。
